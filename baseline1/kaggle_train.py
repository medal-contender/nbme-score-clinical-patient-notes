...
OUTPUT_DIR = f'../checkpoint/deberta/cosineann_sgf/'
...
class CFG:
    ...
    scheduler='CosineAnnealingWarmupRestarts' # ['linear', 'cosine', 'CosineAnnealingLR', CosineAnnealingWarmupRestarts]
...
train = pd.DataFrame()
# for i in range(5):
#     oof = pd.read_pickle(f"/home/ubuntu/hhl/challenge/nbme-score-clinical-patient-notes/input/skf_dataset/skf-dataset-fold{i}.pkl")
#     train = pd.concat([train, oof])
train = pd.read_pickle(f"/home/ubuntu/hhl/challenge/nbme-score-clinical-patient-notes/input/sgf_dataset/NBME_skf_dataset.plk")
train['fold'].unique()

# train = pd.DataFrame()
# for i in range(5):
#     oof = pd.read_pickle(f"../input/testtest/qa-dataset-fold{i}.pkl")
#     train = pd.concat([train, oof])
# train['fold'].unique()
...
# train = pd.DataFrame()
# for i in range(5):
#     oof = pd.read_pickle(f"../input/nbmetraindataset/train_fold{i}.pkl")
#     train = pd.concat([train, oof])
# train['fold'].unique()
...
CFG.max_len = 354 # max(pn_history_lengths) + max(features_lengths) + 3 # cls & sep & sep
...
def train_loop(folds, fold):
    ...
    for epoch in range(CFG.epochs):
        ...
        if best_score < score:
            ...
            patience_cnt += 1
            print('EarlyStopping counter: {} out of {}'.format(patience_cnt, CFG.patience))
